...
(a module carrying its own index, name clash avoidance) are used unchanged.
...
| `loan-approval/pom.xml`, `risk-assessment/pom.xml`                        | `vanillabp-quarkus-support` and the API JAR, never an adapter outside the test scope         |
| `application/pom.xml`                                                     | the two module dependencies, the VanillaBP extension and the BPMS adapter                    |
| `loan-approval/pom.xml`, `risk-assessment/pom.xml`, `banking-api/pom.xml` | the index plugin, which is what makes a JAR's classes visible, as in `module-multi`          |
...
8. Reach a test bean through its methods, never through its fields: what a test holds of a
   bean is the container's client proxy, and a proxy forwards calls, not field access. A stub
   whose recorded calls are read from a field looks empty and the test waits forever.
